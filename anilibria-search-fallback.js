(function(){
  ...
  const genreAliases={
    ...
    'Фантастика':['фантастика','sci-fi','sci fi'], 'Ужасы':['ужасы','horror'], 'Детектив':['детектив','mystery'],
    'Триллер':['триллер','thriller'], 'Спорт':['спорт','sports'], 'Музыка':['музыка','music'], 'Школа':['школа','school'],
  ...
  function normalize(item){
    ...
    return {id:r.id||item?.id||r.alias||item?.alias||title,title,image,rating:r.averageScore||r.rating||item?.rating||'—',votes:r.votes||r.ratingCount||item?.votes||null,year:r.year||r.season?.year||item?.year||null,episodes:r.episodes||r.episodes_count||item?.episodes||0,status:r.status||item?.status||'',duration:r.duration||item?.duration||0,type:r.type||item?.type||'Аниме',genres:Array.isArray(r.genres)?r.genres.map(g=>typeof g==='string'?g:(g?.name||g?.russian||'')).filter(Boolean):[],description:r.description||item?.description||''};
  }
  function textGenres(a){return (a.genres||[]).map(g=>String(g).toLowerCase().trim())}
  function matchesFilters(a,filters){
    const selected=Array.isArray(filters?.genres)?filters.genres.filter(Boolean):[];
    if(selected.length){
      const gs=textGenres(a);
      if(!selected.every(g=>{const aliases=genreAliases[g]||[String(g).toLowerCase()];return aliases.some(x=>gs.some(y=>y.includes(x))) }))return false;
    }
    if(filters?.year&&Number(a.year)!==Number(filters.year))return false;
    if(filters?.format){
      const t=String(a.type||'').toLowerCase();
      const map={Сериал:['tv','сериал','series'],Фильм:['movie','фильм'],OVA:['ova'],ONA:['ona']};
      if(map[filters.format]&&!map[filters.format].some(x=>t.includes(x)))return false;
    }
    if(filters?.rating){const score=Number(a.rating);if(Number.isFinite(score)&&score<Number(filters.rating))return false;}
    return true;
  }
  ...
  async function fallback(query,filters={}){
    ...
    const selected=Array.isArray(filters.genres)?filters.genres.filter(Boolean):[];
    const queries=q?[q]:selected;
    if(!queries.length)return [];
    const chunks=await Promise.all(queries.slice(0,5).map(fetchSearch));
    const seen=new Set();
    return chunks.flat().filter(a=>{const key=String(a.id||a.title).toLowerCase();if(seen.has(key))return false;seen.add(key);return matchesFilters(a,filters)});
  }
  window.searchAnimeApi=async function(query,filters){
    ...
    try{
      const fallbackResult=await fallback(query,filters||{});
